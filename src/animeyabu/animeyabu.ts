...
class Provider {
    ...
    async search(opts: SearchOptions): Promise<SearchResult[]> {
        ...
        $("div.ultimosAnimesHomeItem a").each((_, el) => {
            ...
            if (href && title) {
                // Anime Yabu puts the anime URL directly
    ...
    async findEpisodes(id: string): Promise<EpisodeDetails[]> {
        // id is the url like "https://www.animeyabu.org/animes/boku-no-hero-academia-dublado"
    ...
    async findEpisodeServer(episode: EpisodeDetails | any, _server: string): Promise<EpisodeServer> {
        ...
        const tokenMatches = [...html.matchAll(/foodiesbrazil\.info\/[a-zA-Z0-9_]+\.php\?t=([^"'\s]+)/ig)]
        ...
        for (const token of tokens) {
            try {
                const fetchHeaders = {
                    "referer": "https://www.animeyabu.org",
                    "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
                }

                try {
                    await fetch(`https://foodiesbrazil.info/anf.php?t=${token}`, { headers: fetchHeaders })
                    await fetch(`https://ondeviajar.online/data5.php?token=${token}`, { headers: fetchHeaders })
                    await fetch(`https://ondeviajar.online/data3.php?token=${token}`, { headers: fetchHeaders })
